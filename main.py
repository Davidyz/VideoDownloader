...
def Action(update, context):
    ...
    if is_video:
        ...
        context.bot.sendPhoto(
            chat_id=update.message.chat_id, photo=open(file_path, "rb")
        )
...
def checkDownloaded(context):
    ...
    for job in finished:
        ...
        for i in finished_downloads:
            job.update.message.reply_text(
                text=language["finished_download"]
                .format(finished_downloads[i])
...
def checkProcess(context):
    ...
    for i in finished_processes:
        ...
        context.bot.sendMessage(
            ...
            text=language["process_done"].format(i),
            parse_mode="markdown",
        )
...
def get_downloading(update, context):
    ...
    if result:
        update.message.reply_text(
            "\n".join(
                [language["current_job"]]
                + [i.replace("_哔哩哔哩 (゜-゜)つロ 干杯~-bilibili", "") for i in result]
            )
        )
    else:
...
def send_math(chat_id, string):
    ...
    sympy.preview(
        ...
        viewer="file",
        filename=filename,
        dvioptions=["-D", str(settings.LATEX_DPI)],
    )
    return filename
...
def sys_info(update, context):
    ...
    temperature = round(
        sum(
            [
                psutil.sensors_temperatures()["coretemp"][i][1]
    ...
    message = "CPU utilization: {cpu}%\nMemory utilization: {mem}\nSwap utilization: {swap}\nTemperature: {temp}°C\n".format(
        cpu=cpu, mem=mem, swap=swap, temp=temperature
    )
