...
class Downloader:
    ...
    def download(self, url, path=None, audioOnly=False, site=None):
        ...
        if isinstance(url, str):
            ...
            if site == "youtube":
                try:
                    title = youtube_dl.YoutubeDL(self.__ydl_opts).extract_info(
                        url, download=False
                    )["title"]
                ...
                command: List[str] = [
                    "yt-dlp",
                    "-f",
                    int(not audioOnly) * "bestvideo+" + "bestaudio",
                    url,
                    "-o",
                    self.__ydl_opts["outtmpl"],
                ]
                if settings.PROXY != "" and settings.PROXY.split("://")[0] in (
                    "http",
                    "socks5",
                ):
                ...
                proc = multiprocessing.Process(
                    target=biget.Video.download,
                    args=(video,),
                    kwargs={
                        "pages": range(video.page_num),
                        "path": self.defaultPath,
                        "keep": False,
                    },
                )
    ...
    def check_finished(self):
        ...
        for i in self.__inProgress:
            try:
                ...
                if (
                    psutil.Process(i).name() == self.downloading[i]
                    and psutil.Process(i).status() != "zombie"
                ):
                    pass
                else:
    ...
    def transcode(self, path, title=None):
        ...
        for i in files:
            ...
            command = 'ffmpeg -v 0 -i "{source}" -c:v libx265 -c:a copy -q 0 -y "{target}"'.format(
                source=i, target=new_filename
            )
