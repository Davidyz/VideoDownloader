...
class Process:
    def __init__(self, proc, name=None):
        if isinstance(proc, int) or (
            isinstance(proc, str) and all([i in "0123456789" for i in proc])
        ):
            self.pid = int(proc)
            if name != None:
                self.proc_name = name
    ...
    def isAlive(self):
        return psutil.pid_exists(self.pid) and (
            psutil.Process(self.pid).name() == self.proc_name
        )
    ...
    def write(self):
        ...
        if os.path.isfile(os.sep.join([settings.CWD, ".process"]).replace("//", "/")):
            with open(
                os.sep.join([settings.CWD, ".process"]).replace("//", "/"), "r"
            ) as fin:
                for i in fin.readlines():
        ...
        with open(
            os.sep.join([settings.CWD, ".process"]).replace("//", "/"), "w"
        ) as fin:
            for i in current:
                fin.write(i + "\n")
